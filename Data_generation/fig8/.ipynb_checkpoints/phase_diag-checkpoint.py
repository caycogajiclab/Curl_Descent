...
#        ])
    ...
    if modification_type == 'col':
        # Similarly, define the list of initial student parameters
        student_params_ini_mat.append([
            {'W1': 1e-15+teacher_params_mat[j][0]['W1'].copy(), 'W2': teacher_params_mat[j][0]['W2'].copy(), 'D1': np.eye(input_dim), 'D2': np.eye(student_size)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': np.eye(input_dim), 'D2': np.eye(student_size)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': Ds1, 'D2': Ds2},
            {'W1':1e-15+teacher_params_mat[j][0]['W1'].copy(), 'W2': teacher_params_mat[j][0]['W2'].copy(), 'D1': Ds1, 'D2': Ds2}
        ])
    elif modification_type == 'row':
        student_params_ini_mat.append([
            {'W1': 1e-15+teacher_params_mat[j][0]['W1'].copy(), 'W2': teacher_params_mat[j][0]['W2'].copy(), 'D1': np.eye(student_size), 'D2': np.eye(output_dim)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': np.eye(student_size), 'D2': np.eye(output_dim)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': Ds1, 'D2': Ds2},
            {'W1': 1e-15+teacher_params_mat[j][0]['W1'].copy(), 'W2':teacher_params_mat[j][0]['W2'].copy(), 'D1': Ds1, 'D2': Ds2}
        ])
...
# Save using pickle
with open(f'/shared/projects/project_curlDynamics/clean_code/normalized/phase_diag_L2_tanh/run_{nb_AH_layer1}-{input_dim}__{nb_AH_layer2}-{student_size}__{task_id}.pkl', 'wb') as f:
