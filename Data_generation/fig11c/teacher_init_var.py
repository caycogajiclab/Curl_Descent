...
teacher_init_range = float(arguments[17])
...
for j in range(nb_init):
    ...
    if initialization_type == 'normal':
        ...
        Wt1 = np.random.normal(0, teacher_init_range/(np.sqrt(input_dim)), size=(teacher_size, input_dim))
        Wt2 = np.random.normal(0, teacher_init_range/(np.sqrt(teacher_size)), size=(output_dim, teacher_size))
        ...
        Wt1 = teacher_init_range * np.random.uniform(-1/(np.sqrt(input_dim)), 1/(np.sqrt(input_dim)), size=(teacher_size, input_dim))
        Wt2 = teacher_init_range * np.random.uniform(-1/(np.sqrt(teacher_size)), 1/(np.sqrt(teacher_size)), size=(output_dim, teacher_size))
...
params_dic = {'nb_data_points':nb_data_points, 'nb_test_data_points':nb_test_data_points,
             ...
             'teacher_init_range':teacher_init_range}
...
# Save using pickle
with open(f'/shared/projects/project_curlDynamics/clean_code/not_normalized/rebuttal/teacher_init_var/run_{nonlinearity}_{initialization_type}_{task_id}.pkl', 'wb') as f:
    pickle.dump((params_dic, np.array(input_list), np.array(input_test_list), Ws1_hist_dic, Ws2_hist_dic, error_hist_dic, test_error_hist_dic, teacher_params_mat), f)