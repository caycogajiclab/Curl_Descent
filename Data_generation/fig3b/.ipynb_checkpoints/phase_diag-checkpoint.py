...
class TeacherStudentLearning:
    ...
    def learning_simu_batch(self, input_list, input_test_list, batch_size, nb_epochs, B, learning_rate, plasticity, modification_type, nonlinearity, normalize_weights=True, batch_norm=True,  test_set=False):
        ...
        log_indices_whist = np.arange(nb_epochs-1000, nb_epochs)
        ind = 1
        ind_whist = 1
        ...
        for epoch in range(nb_epochs):
            ...
            if epoch == log_indices_whist[ind_whist]:
                # Record history
...
for j in range(nb_init):
    ...
    Wt1 *= np.sqrt(teacher_size)/np.linalg.norm(Wt1)
    Wt2 *= 1/np.linalg.norm(Wt2)
...
for j in range(nb_init):
    ...
    if modification_type == 'col':
        ...
        student_params_ini_mat.append([
            {'W1': teacher_params_mat[j][0]['W1'].copy(), 'W2': 1e-10+teacher_params_mat[j][0]['W2'].copy(), 'D1': np.eye(input_dim), 'D2': np.eye(student_size)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': np.eye(input_dim), 'D2': np.eye(student_size)},
            {'W1': Ws1_ini.copy(), 'W2': Ws2_ini.copy(), 'D1': Ds1, 'D2': Ds2},
            {'W1': teacher_params_mat[j][0]['W1'].copy(), 'W2': 1e-10+teacher_params_mat[j][0]['W2'].copy(), 'D1': Ds1, 'D2': Ds2},
            
        ])
    elif modification_type == 'row':
        student_params_ini_mat.append([
            {'W1': teacher_params_mat[j][0]['W1'].copy(), 'W2': 1e-10+teacher_params_mat[j][0]['W2'].copy(), 'D1': np.eye(student_size), 'D2': np.eye(output_dim)},
...
### computing the q order parameter
...
for trial in range(nb_init):
    ...
    all_w_hist = np.concatenate((w1_hist.reshape(999, student_size*input_dim ), 
                           w2_hist.reshape(999, student_size)), axis = 1)
    ...
    qtrial = np.nanmean(var_w_hist)/np.nanvar(all_w_mean_hist)
...
# Save using pickle
with open(f'/shared/projects/project_curlDynamics/clean_code/normalized/transition_to_chaos/run_{nb_AH_layer1}-{input_dim}__{nb_AH_layer2}-{student_size}__{task_id}.pkl', 'wb') as f:
